...
class Client:
    ...
    def send_meeting_details(self):
        ...
        if response.status_code == 201:
            print(colored("\nSuccessfully submitted a booking request for room: {}", 'green').format(
                self.meetingRoomData["room_no"]))
        else:
        ...
        while True:
            ...
            for l in range(0, len(bookings_list)):
                ...
                if booking_list_item["booking_status"].upper() == "TENTATIVE":
                    ...
                    print(colored('{}\t{}\t{}\t{}\t{}\t{}', 'yellow').format(booking_list_item["id"],
                                                                             booking_list_item["room_no"],
                                                                             booking_list_item["booking_date"],
                                                                             booking_list_item["start_time"], end_time,
                                                                             booking_list_item["booking_status"]))
                elif booking_list_item["booking_status"].upper() == "COMMITTED":
                    print(colored('{}\t{}\t{}\t{}\t{}\t{}', 'green').format(booking_list_item["id"],
                                                                            booking_list_item["room_no"],
                                                                            booking_list_item["booking_date"],
                                                                            booking_list_item["start_time"], end_time,
                                                                            booking_list_item["booking_status"]))
                else:
                    print(colored('{}\t{}\t{}\t{}\t{}\t{}', 'red').format(booking_list_item["id"],
                                                                          booking_list_item["room_no"],
                                                                          booking_list_item["booking_date"],
                                                                          booking_list_item["start_time"], end_time,
                                                                          booking_list_item["booking_status"]))
